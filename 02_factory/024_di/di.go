...
type Container struct {
	providers map[reflect.Type]provider      // 假设一种类型只能有一个 provider 提供
	results   map[reflect.Type]reflect.Value // 缓存以生成的对象
}
...
func (c *Container) Invoke(function interface{}) error {
	...
	vt := v.Type() // v.Kind() 一定是 func

	var err error
	params := make([]reflect.Value, vt.NumIn())
	for i := 0; i < vt.NumIn(); i++ { // NumIn 是 vt 的入参个数
		params[i], err = c.buildParam(vt.In(i)) // 获取 v 的参数
		if err != nil {
			return err
		}
	}
	v.Call(params) // 运行函数 v，参数是 params[*]
	return nil
}
